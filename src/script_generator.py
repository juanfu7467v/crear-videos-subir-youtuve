...
class ScriptGenerator:
    def __init__(self, api_key: str):
        ...
        # Modelo estable actual de Gemini
        # Usamos el alias oficial que siempre apunta a la última versión estable
    ...
    def generate_full_script(self, trend_data: dict) -> dict:
        ...
        try:
            ...
            # Limpieza de formato por si Gemini envía markdown
            cleaned_text = re.sub(
                r'^```json\s*|\s*```$',
                '',
                text_response.strip(),
    ...
    def _get_fallback(self, topic: str):
        ...
        return {
            ...
            "full_script": (
                f"¡Detente! ¿Alguna vez te has preguntado sobre {topic}? "
                f"En este video de El Tío Jota, vamos a desglosar los datos más "
                f"alucinantes que cambiarán tu forma de ver este tema. "
                f"Acompáñame en este recorrido rápido y lleno de información curiosa. "
                f"No olvides suscribirte para más contenido fascinante cada día."
            ),
            "voice": "random",
            "keywords": f"{topic}, viral, datos curiosos, historia",
            "description": (
                f"Descubre los secretos de {topic} con El Tío Jota. "
                f"Un video corto pero lleno de información impactante."
            ),
